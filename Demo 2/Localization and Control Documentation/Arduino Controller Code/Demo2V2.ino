...
void setup() {
  ...
  // Initializes Raspberry Pi communication.
  Wire.begin(MY_ADDR);
...
void loop() {  
  ...
  // Turns until a marker is detected. If a marker is detected, goes to the next state.
    ...
    if (detected == true) {
      ...
      state = DETECTED_STATE;
      startMove = false;
      desiredAngle = robotAngle + currentAngle*PI/180;
      desiredDistance = robotDistance;
    }
  ...
  // web camera (angle and distance) to move to the target
    ...
    // Reads from Raspberry Pi to determine current angle.
    if (msgLength > 0) {
      if (offset == 1) {
        digitalWrite(LED_BUILTIN,instruction[0]);
      }
      ...
      TARGET_ANGLE_DEG = robotAngle + currentAngle*PI/180;
      TARGET_DISTANCE = 7;
    ...
    // Resets the communication if web camera no longer reads anything.
    // Will also go back to location state.
    else {
      undetectCounter++;
...
void printReceived() {
  for (int i = 0; i < msgLength; i++) {
    ...
    // Due to limitations in the I2C, the Arduino receives an
    // angle multiplied by 100 and in 2s complement. This
    // converts the code correctly.
    if ((int)currentAngle & 0x80) {
      currentAngle = -((~(int)currentAngle + 1) & 0x7F);
    }
...
void request() {
 ...
 // Receives data from Raspberry Pi and stores in array.
 void receive() {
