// Demo 1 Robot Localization and Control V2
// ================
...
float desiredAngleVel = PI/2;
float desiredDistVel = 2;
...
const float TARGET_ANGLE_DEG = 90;
...
double encClicksToFeet = 2*PI*0.25/3200;
...
void loop() {  
  ...
  // for the feedback control loop. These include gains and integral
  // variables for the angle and distance
  float Kp_angle = 6;
  float Kp_distance = 6;
  float Ki_angle= 8;
  float Ki_distance = 5.5;
  float angleVelKp = 6;
  float distanceVelKp = 5;
  float Battery_Voltage = 7.8;
  float maxDistVel = 1;

  // Variables to calculate the ramp function input rises.
  float distanceRise = TARGET_DISTANCE/2000 * desired_Ts_ms;
  float angleRise = TARGET_ANGLE_DEG*PI/180/1250 * desired_Ts_ms;
  ...
  // Anti-windup for the distance velocity to be fixed within -1 and 1.
