...
const int LINE_UP_STATE = 2;
const int MOVE_STATE = 3;
const int TURN_90_STATE = 4;
const int CIRCLE_STATE = 5;
...
void loop() {
  ...
  // Moves the robot a set distance towards a marker.
    ...
    float addedCalibration = 0.26;
    float multipliedCalibration = 0.89;
    TARGET_DISTANCE = receivedDistance * multipliedCalibration + addedCalibration;
  ...
  // clockwise to prepare for circle.
    ...
    if (((current_time - startCircleTime) > 1) && (task2 == true)) {
      desiredAngle = 0;
      desiredDistance = 0;
      state = CIRCLE_STATE;
  ...
  // Makes the robot move in a circle at some radius.
    ...
    if ((current_time - startCircleTime) > 3.87) {
      start = false;
      state = IDLE_STATE;
      counter = 0;
      desiredAngle = 0;
  ...
  for (int i = 0; i < msgLength; i++) {
    ...
    if (i%2 == 0){
      ...
      currentAngle = currentAngle / 10;
        ...
        receivedDistance = instruction[i]/10;
    }
  }
}
